...
source('openness_three_sim_no.R')
...
for (i in 1:nruns) {
  ...
  theta <- theta_temp/(theta_temp + beta_rel_temp + beta_ip_temp)
  beta_rel_no <- beta_rel_temp/(theta_temp + beta_rel_temp + beta_ip_temp)
  beta_ip_no <- beta_ip_temp/(theta_temp + beta_rel_temp + beta_ip_temp)
  
  # Simulate
  sim_data <- openness_three_sim(theta, beta_rel_no, beta_ip_no,
  ...
  data <- list("bin_open", "nsubjects", "bin_rel", "bin_ip")
  params <- c("theta", "theta_q_no", "beta_rel_no", "beta_ip_no")
  
  openinfo_qual <- jags.parallel(data, inits=NULL, params,
                                 model.file ="openness_jags_no.txt",
                                 n.chains=1, n.iter=5000, n.burnin=1000, n.thin=1, n.cluster=1)
  
  # Parameter recovery
  trueTheta[i] <- theta
  trueBeta_rel[i] <- beta_rel_no
  trueBeta_ip[i] <- beta_ip_no
  
  X <- openinfo_qual$BUGSoutput$sims.list
  inferredTheta[i] <- MPD(X$theta)
  inferredBeta_rel[i] <- MPD(X$beta_rel_no)
  inferredBeta_ip[i] <- MPD(X$beta_ip_no)
}
