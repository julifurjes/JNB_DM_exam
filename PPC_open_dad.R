...
# Posterior density of the parameters (of simulated data)
par(mfrow=c(2,2))
plot(density(openinfo_qual$BUGSoutput$sims.list$theta))
plot(density(openinfo_qual$BUGSoutput$sims.list$beta_rel_dad))
plot(density(openinfo_qual$BUGSoutput$sims.list$beta_ip_dad))
plot(density(openinfo_qual$BUGSoutput$sims.list$beta_dad))

# Posterior predictive checks of descriptive accuracy

# "Probability" is the theta_q posterior
# theta_q decides for each participant whether to be open or not
theta_q_post <- openinfo_qual$BUGSoutput$sims.list$theta_q_dad #df is 98 x 4000 (bc we made 5000 iterations, but rejected 1000)
