openness_three_sim <- function(theta, beta_rel, beta_, beta_ip, 
                         bin_rel, bin_, bin_ip, nsubjects) {
  ...
  for (p in 1:nsubjects) {
    ...
    theta_q[p] <- (1-(beta_rel+beta_+beta_ip))*theta + beta_rel*bin_rel[p] + beta_*bin_[p] + beta_ip*bin_ip[p]
